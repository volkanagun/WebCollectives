package data.crawler.web;

import java.io.Serializable;

/**
 * Created by wolf on 02.07.2015.
 */
public class LookupOptions implements Serializable {

    public static String EMPTYDOMAIN = "";
    public static String EMPTY = "";

    public static String URL = "URL";
    public static String CONTAINER = "CONTAINER";
    public static String LOOKUP = "LOOKUP";

    public static String TEXT = "TEXT";
    public static String DATE = "DATE";
    public static String TAG = "TAG";

    public static String MAINPAGE = "MAINPAGE";
    public static String AUTHOR = "AUTHOR";
    public static String AUTHORLINK = "AUTHORLINK";
    public static String BOOKLINK = "BOOKLINK";
    public static String AUTHORNAME = "AUTHORNAME";
    public static String SKIP = "SKIP";

    public static String ARTICLE = "ARTICLE";
    public static String ARTICLEID = "ARTICLEID";
    public static String ARTICLETEXT = "ARTICLETEXT";
    public static String ARTICLEPARAGRAPH = "ARTICLEPARAGRAPH";
    public static String ARTICLELINK = "ARTICLELINK";

    public static String ARTICLEDOC = "ARTICLE-DOC";
    public static String BLOGDOC = "BLOG-DOC";

    public static String TWEETDOC = "TWEET-DOC";
    public static String REUTERSDOC = "REUTERS-DOC";
    public static String PANDOC = "PAN-DOC";
    public static String SIKAYETVARDOC = "SIKAYETVAR-DOC";


    public static String ARTICLELINKCONTAINER = "ARTICLELINKCONTAINER";
    public static String ARTICLETITLE = "ARTICLETITLE";
    public static String ARTICLERATING = "ARTICLERATING";
    public static String TWEET = "TWEET";
    public static String TWEETTEXT = "TWEETTEXT";
    public static String BLOG = "BLOG";

    public static String GENRE = "GENRE";
    public static String GENRETEXT = "GENRETEXT";
    public static String LANGUAGE = "LANGUAGE";
    public static String FANFIC = "FANFIC";
    public static String TOPIC = "TOPIC";


    public static String URLPAGE = "URLPAGE";
    public static String UTF8 = "UTF-8";


    public static String MAINDIRECTORY = "resources";
    public static String HTMLDIRECTORY = MAINDIRECTORY + "/htmls/";
    public static String POLITICSDIRECTORY = "resources/politics";
    public static String RELIGIONDIRECTORY = "resources/religion";
    public static String TECHDIRECTORY = "resources/technology";
    public static String BLOGDIRECTORY = "resources/blogs-turkish";
    public static String FORUMENDIRECTORY = "resources/forums-english";

    public static String FANFICDIRECTORY = "resources/fanfics";
    public static String BLOGENGDIRECTORY = "resources/blogs-english";
    public static String TEXTENGDIRECTORY = "resources/texts-english";
    public static String SIKAYETVARGDIRECTORY = "resources/sikayet-var";

    public static String BLOGTRYDIRECTORY = "resources/blogs-trial";
    public static String TESTDIRECTORY = "resources/tests";
    public static String ARTICLEDIRECTORY = "resources/articles";
    public static String TURKISHARTICLEDIRECTORY = "resources/articles-turkish";
    public static String TWEETDIRECTORY = "resources/tweets";
    public static String REUTERSDIRECTORY = "resources/reuters";
    public static String REUTERSSOURCEDIRECTORY = "resources/source/";

    public static String PANLARGEDIRECTORY = "resources/pan/raw-large/";
    public static String PANLARGETESTDIRECTORY = "resources/pan/raw-large-test/";
    public static String PANLARGETRAINDIRECTORY = "resources/pan/raw-large-train/";

    public static String PANLARGESOURCEDIRECTORY = "resources/pan/sources-large/";
    public static String PANLARGETESTSOURCEDIRECTORY = "resources/pan/sources-large-test/";
    public static String PANLARGETRAINSOURCEDIRECTORY = "resources/pan/sources-large-train/";

    public static String PANSMALLDIRECTORY = "resources/pan/raw-small/";
    public static String PANSMALLTESTDIRECTORY = "resources/pan/raw-small-test/";
    public static String PANSMALLTRAINDIRECTORY = "resources/pan/raw-small-train/";

    public static String PANSMALLSOURCEDIRECTORY = "resources/pan/sources-small/";
    public static String PANSMALLTESTSOURCEDIRECTORY = "resources/pan/sources-small-test/";
    public static String PANSMALLTRAINSOURCEDIRECTORY = "resources/pan/sources-small-train/";


    public static String GENRESPORTS = "sports";
    public static String GENRETRAVEL = "travel";
    public static String GENRECULTURE = "culture";
    public static String GENRETECH = "technology";
    public static String GENREPOLITICS = "politics";
    public static String GENRESENTIMENT = "sentiment";


}
